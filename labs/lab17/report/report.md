...
Использую многоканальное устройство `STORAGE`, которое содержит 2 канала: заявки типа `A` и `B` занимают по 1 каналу, а заявка типа `C` - 2 (рис. @fig:001).
...
Загрузка системы составляет `0.994`.
...
- Блок `GATE` для перехода к посадке в случае, если ВПП свободна[@studfile_gpss]
